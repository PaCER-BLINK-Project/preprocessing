...
Visibilities<T> reorder_visibilities(const Visibilities<T>& vis, const antenna_mapping_t& mapping){
    ...
    const int n_pols {2};
    const int n_channels {vis.nFrequencies};
    const int n_baselines {(n_antennas + 1) * (n_antennas / 2)};
    const int matrix_size {n_baselines * n_pols * n_pols * 2};
    const int n_intervals {vis.integration_intervals()};

    T* final_new_vis {new T[ n_intervals * n_channels * matrix_size]};

    for(int interval {0}; interval < n_intervals; interval++){
        T* new_vis {final_new_vis + interval * matrix_size * n_channels};
        T* curr_vis {vis.data + interval * matrix_size * n_channels};
            ...
            for(int src_baseline {0}; src_baseline < n_baselines; src_baseline++){
                ...
                for(int src_pol1 {0}; src_pol1 < 2; src_pol1++){
                    for(int src_pol2 {0}; src_pol2 < 2; src_pol2++){
                        ...
                        const int src_idx {src_baseline * 8 +  src_pol1 * 4 + src_pol2 * 2};
                        int new_baseline;
                        int new_idx;
                        if(dest_a2 > dest_a1){
                            // we need to transpose polarisations
                            new_baseline =  get_baseline_from_antenna_pair(dest_a2, dest_a1);
                            new_idx = new_baseline * 8 + dest_p2 * 4 + dest_p1 * 2;
                        }else{
                            new_baseline =  get_baseline_from_antenna_pair(dest_a1, dest_a2);
                            new_idx = new_baseline * 8 + dest_p1 * 4 + dest_p2 * 2;
                        }
                        // copy the real part
                        new_vis[ch, new_idx] = curr_vis[ch, src_idx];
                        // now the imaginary part, and correct if necessary
                        new_vis[ch, new_idx + 1] = to_conjugate ? (-curr_vis[ch, src_idx + 1]) : curr_vis[ch, src_idx + 1];
                    }
                }
            }
        }
